...
class ViewsController {
    ...
    renderProfileAdmin(req, res) {
        try {
            ...
            logger.error('Error rendering profile admin page:', error);
            res.render("page_not_found");
        }
    }
    ...
    async renderAdminDashboard(req, res) {
        try {
            ...
            const vehiclesResult = await VehicleRepository.getVehicles({ page: currentPage });
            const vehicles = vehiclesResult.docs;
            const DisabledPersonResult = await DisabledPersonRepository.getDisabledPersons({ page: currentPage });
            const disabledPersons = DisabledPersonResult.docs;
            ...
            const companiesConfig = await CompanyConfigRepository.getAllCompanyConfigs();
            ...
            res.render('admin_dashboard', {
                ...
                disabledPersonsPagination: {
                    page: DisabledPersonResult.page,
                    totalPages: DisabledPersonResult.totalPages,
                    hasNextPage: DisabledPersonResult.hasNextPage,
                    hasPrevPage: DisabledPersonResult.hasPrevPage,
                },
                productosPagination,
            });
        } catch (error) {
            logger.error('Error rendering admin dashboard page:', error);
            res.render("page_not_found");
        }
    }
